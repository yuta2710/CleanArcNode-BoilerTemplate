import { ZERO, ValidationType, AppError } from "./../../../../core";
...
export class PaginationDto implements CoreDto<PaginationDto> {
  ...
  public validate(dto: PaginationDto): void {
    ...
    if (isNaN(dto.page) || isNaN(dto.limit)) {
      errors.push({
        fields: ["page", "limit"],
        constraint: "Page and limit must be numbers",
      });
    }
    ...
    if (dto.page <= ZERO) {
      errors.push({
        fields: ["page"],
        constraint: "Page must be greater than zero",
      });
    }
    ...
    if (dto.limit <= ZERO) {
      errors.push({
        fields: ["limit"],
        constraint: "Limit must be greater than zero",
      });
    }
